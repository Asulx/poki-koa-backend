...
class Cuna(models.Model):
    ...
    spo2 = models.IntegerField(
        null=True, blank=True, help_text="Saturación de oxígeno en sangre (%)"
    )
    temperatura = models.FloatField(
        null=True, blank=True, help_text="Temperatura corporal en grados Celsius (°C)"
    )
    ...
    via_iv_activa = models.BooleanField(
        default=False, help_text="True si la vía intravenosa está activa"
    )

    # Se actualiza automáticamente cada vez que se guarda un cambio en la cuna
    ultima_actualizacion = models.DateTimeField(
        auto_now=True, help_text="Fecha y hora de la última actualización de datos"
    )
...
class Medicamento(models.Model):
    ...
    ESTADO_CHOICES: ClassVar = [
        ("Administrado", "Administrado"),
        ("Pendiente", "Pendiente"),
    ]

    VIA_CHOICES: ClassVar = [
        ("IV", "Intravenosa (IV)"),
        ("IM", "Intramuscular (IM)"),
        ("ET", "Endotraqueal (ET)"),
        ("VO", "Vía Oral (VO)"),
    ...
    dosis = models.CharField(
        max_length=50, help_text="Cantidad y unidad de la dosis (ej: 5 mg/kg)"
    )
    via = models.CharField(
        max_length=2, choices=VIA_CHOICES, help_text="Vía de administración del fármaco"
    )
...
class PlanCuidado(models.Model):
    ...
    frecuencia = models.CharField(max_length=50, help_text="Ej: Continuo, c/3h, c/24h")
    estado = models.CharField(
        max_length=20,
        choices=ESTADO_CHOICES,
        default="Activo",
...
class Alerta(models.Model):
    ...
    mensaje = models.CharField(
        max_length=255, help_text="Descripción o mensaje detallado de la alerta"
    )
